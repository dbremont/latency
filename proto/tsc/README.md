...
- We need to be able to compute statistis over temporal windows of various sizes 1min, 5m based on 1min resolution computaiton, that is take the data build 1 minute resolution based values -> and used those to derived the new resolutions.
...
- P50 Average (Median)
- P75 Average
- Variance of Log-Latency
- Max Value

Evaluation metrics:

- Consistency:  Check tha the statistics for one one resolution build on the based 1min resolution is consisteny from the resoution statistics builds from the raw data.
- Per every resolution -  and for every window based on that resolution compute the metrics based on the 1min resolution metris data (derived the new metrics from the 1 minutes ones) - than compared this for the metrics - for the window derived from the raw data - report the deviation - than report the sum of all the deviation, and the number of window

See the current computation:
